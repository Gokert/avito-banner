...
type ICore interface {
	GetUserBanner(ctx context.Context, tagId uint64, featureId uint64, lastVersion bool) (*models.UserBanner, bool, error)
...
func (c *Core) CheckFeature(ctx context.Context, featureId uint64) (bool, error) {
	find, err := c.banner.CheckFeature(ctx, featureId)
	if err != nil {
...
func (c *Core) GetUserBanner(ctx context.Context, tagId uint64, featureId uint64, lastVersion bool) (*models.UserBanner, bool, error) {
	find, err := c.CheckFeature(ctx, featureId)
	if err != nil {
	...
	findBanner, err := c.banner.CheckBanner(ctx, featureId)
	if err != nil {
	...
	banner, err := c.banner.GetUserBanner(ctx, tagId, featureId, lastVersion)
	if err != nil {
...
func (c *Core) GetBanners(ctx context.Context, tagId uint64, featureId uint64, getAllBanners bool, offset uint64, limit uint64) (*[]models.BannerResponse, error) {
	banners, err := c.banner.GetBanners(ctx, tagId, featureId, getAllBanners, offset, limit)
	if err != nil {
...
func (c *Core) CheckBanner(ctx context.Context, bannerId uint64) (bool, error) {
	check, err := c.banner.CheckBanner(ctx, bannerId)
	if err != nil {
...
func (c *Core) DeleteBanner(ctx context.Context, bannerId uint64) (bool, error) {
	res, err := c.banner.CheckBanner(ctx, bannerId)
	if err != nil {
	...
	_, err = c.banner.DeleteBanner(ctx, bannerId)
	if err != nil {
...
func (c *Core) UpdateBanner(ctx context.Context, banner *models.BannerRequest) (bool, error) {
	if banner.FeatureId == 0 || len(banner.TagIds) == 0 {
		return false, fmt.Errorf("feature id is required")
	}

	res, err := c.banner.CheckBanner(ctx, banner.BannerId)
	if err != nil {
		return false, fmt.Errorf("update banner error: %s", err.Error())
	}
	...
	_, err = c.banner.UpdateBanner(ctx, banner)
	if err != nil {
		return false, fmt.Errorf("update banner error: %s", err.Error())
	}
...
func (c *Core) CreateBanner(ctx context.Context, banner *models.BannerRequest) error {
	if banner.FeatureId == 0 || len(banner.TagIds) == 0 {
		return fmt.Errorf("feature id is required")
	}

	err := c.banner.CreateBanner(ctx, banner)
	if err != nil {
